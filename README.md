...
**产物**：一张可快速提取、传播和执行的 Obsidian 原子卡。先用来源审计找到用户真正的核心，防止后续例子和 AI 补充反客为主；再生成一个可作为标题或开头的忠实钩子，按机制匹配并核验已有原则或模型，最后写出原始想法梳理、洞见、真实规律、关键杠杆、边界与最小 SOP。SOP 必须包含输入、动作、产物和完成信号。
